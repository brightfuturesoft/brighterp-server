...
const { attribute_collection } = require("../../../collection/collections/item/items");

const get_attribute = async (req, res, next) => {
      try {
            ...
            const manufactures = await attribute_collection.find({ workspace_id, delete: { $ne: true } }).toArray();
            return response_sender({
                  res,
                  status_code: 200,
                  error: false,
...
const create_attribute = async (req, res, next) => {
      try {
            ...
            const find_manufacture = await attribute_collection.findOne({ code: input_data.code,workspace_id,delete:false });
            if (find_manufacture) {
                  return response_sender({
                        res,
                        status_code: 400,
            ...
            const result = await attribute_collection.insertOne(updated_data);
            return response_sender({
                  res,
                  status_code: 200,
                  error: false,
...
const delete_attribute = async (req, res, next) => {
      try {
            const input_data = req.body;
            const workspace_id = req.headers.workspace_id;
            const check_workspace = await workspace_collection.findOne({ _id: new ObjectId(workspace_id) });
...
module.exports = { create_attribute, get_attribute, update_attribute, delete_attribute }
