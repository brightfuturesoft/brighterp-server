...
const { newsletter_collection } = require("../../../collection/collections/ecommerce/ecommerce");
...
const get_newsletters = async (req, res, next) => {
  try {
    ...
    const newsletters = await newsletter_collection.find({ workspace_id, delete: { $ne: true } }).toArray();
    return response_sender({
      res,
      status_code: 200,
      error: false,
