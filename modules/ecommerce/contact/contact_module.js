...
const { contact_collection } = require("../../../collection/collections/ecommerce/ecommerce");
...
const get_contacts = async (req, res, next) => {
  try {
    ...
    const contacts = await contact_collection.find({ workspace_id, delete: { $ne: true } }).toArray();
    return response_sender({
      res,
      status_code: 200,
      error: false,
