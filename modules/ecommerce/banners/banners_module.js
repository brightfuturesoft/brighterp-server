...
const { banners_collection } = require("../../../collection/collections/ecommerce/ecommerce");


// GET Banners
const get_banners = async (req, res, next) => {
  try {
    ...
    const banners = await banners_collection.find({ workspace_id, delete: { $ne: true } }).toArray();
    return response_sender({
      res,
      status_code: 200,
      error: false,
...
const create_banner = async (req, res, next) => {
  try {
    ...
    const result = await banners_collection.insertOne(updated_data);
...
const update_banner = async (req, res, next) => {
  try {
    ...
    const result = await banners_collection.updateOne(
